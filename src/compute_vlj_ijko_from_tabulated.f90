subroutine compute_vlj_ijko_from_tabulated ( nrgrid , drgrid2 , nb_id_solv , nb_id_mol )
use precision_kinds, only: dp,i2b
use system, only: nfft1 , nfft2 , nfft3 , nb_omega,deltax,deltay,deltaz,nb_solute_sites,eps_mol,x_mol,y_mol,z_mol,Lx,Ly,Lz&
                   ,sig_solv,sig_mol , nb_species, id_mol, chg_mol , chg_solv, eps_solv
...
if ( sum ( sig_solv ( : ) ) /= sig_solv ( 1 ) ) then
  write(*,*)'problem in vext_generator.f90'
  write(*,*)'compute_vlj_ijko_from_tabulated IS ONLY VALID FOR SOLUTES WITH 1 LJ SITE'
  write(*,*)'stop'
  stop
end if
! compute lennard jones potential at each position and for each orientation, for each species => Vext_lj ( i , j , k , omega , species ) 
...
if( min(Lx,Ly,Lz)<= 2.5_dp*max(maxval(sig_mol),maxval(sig_solv))) then
  write(*,*)'max(maxval(sig_mol),maxval(sig_solv)) = ' , max(maxval(sig_mol),maxval(sig_solv))
  write(*,*)'problem detected in compute_vlj_ijko_from_tabulated.f90'
  write(*,*)'the supercell is too small to use the minimum image convention with such large sigma values'
  stop
end if
  ...
  do j=1,nfft2
    ...
    do i=1,nfft1
      ...
      do n=1,nb_solute_sites
        ...
        ! use all images
        !do px = -1 , 1
        !  x_nm2 = (x_grid-(x_mol(n)+px*Lx))**2
        !  do py = -1 , 1
        !    y_nm2 = (y_grid-(y_mol(n)+py*Ly))**2
        !    do pz = -1 , 1
        !      z_nm2 = (z_grid-(z_mol(n)+pz*Lz))**2
 !       if (periodicx) then
 ! dx = x(j) - x(i)
 ! if (abs(dx) >   xsize*0.5) dx = xsize - dx!
!endif
             dx= x_grid-x_mol(n)
             
             if ( dx > 0.5_dp*Lx) then
             dx= dx - Lx
             end if
             if ( dx < -0.5_dp*Lx) then
             dx= Lx + dx
...
open(11,file='Vlj_aumilleiu.dat')
do i=1,nfft3
...
end subroutine compute_vlj_ijko_from_tabulated
