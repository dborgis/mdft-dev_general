...
SUBROUTINE init_external_potential
    ...
    ! electrostatics
    IF ((input_log('point_charge_electrostatic') .OR. input_log('direct_sum')).AND. input_log('poisson_solver')) THEN
    ...
    IF (input_log('point_charge_electrostatic') .OR. input_log('direct_sum')) THEN ! Charges : treatment as point charges
