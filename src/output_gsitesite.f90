subroutine output_gsitesite
     ...
     !write(*,*) solvent(1)%n0/solvent(1)%rho0
     nx = grid%nx
     ny = grid%ny
     nz = grid%nz
     no = grid%no
     ...
     do n=1,size(solute%site)
       do s=1,size(solvent) ! loop over solvents
         do ss=1,size(solvent(s)%site) ! loop over all sites of the selected solvent
             ...
             do j=1,jmax
               do i=1,imax
                 do io=1,no
                     ...
                     g(binr) = g(binr) + grid%w(io)*solvent(s)%xi(io,i,j,k)**2/sum(grid%w)
           ...
           do binr=1,binrmax
             if( abs(g(binr)) > epsdp ) write(124,*) (binr-0.5)*dr, g(binr)/g(binrmax)  ! PROBLEME ICI: TROUVER LA BONNE NORMALISATION !!!!!
           end do
           write(124,*)
         end do
       end do
