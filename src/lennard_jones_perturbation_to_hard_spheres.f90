...
deltav = Lx * Ly * Lz / real ( nfft1 * nfft2 * nfft3 , kind = dp ) ! integration factor
...
if ( .not. allocated ( v_perturbation_k ) ) then
  ...
  do l = 1 , nf1 + 1
    ...
    kx2 = ( twopiOLx * real ( m1 , kind = dp ) ) ** 2
    ...
    do m = 1 , nfft2
      ...
      ky2 = ( twopiOLy * real ( m2 , kind = dp ) ) ** 2
      ...
      do n = 1 , nfft3
        ...
        kz2 = ( twopiOLz * real ( m3 , kind = dp ) ) ** 2
...
contains
  ...
  dx = ( borne_sup - borne_inf ) / real ( nstep , kind = dp )
  ...
  do i = 1 , nstep
    ...
    ri = borne_inf + real ( i - 1 , kind = dp ) * dx
