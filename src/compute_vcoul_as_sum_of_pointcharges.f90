...
! Returns the direct sum of all qi*qj/rij 
    ...
    USE system              ,ONLY: spaceGrid, nb_species, solventSite, soluteSite
    ...
    REAL(dp), DIMENSION( SIZE(solventSite), molRotGrid%n_angles, angGrid%n_angles) :: xmod, ymod, zmod
    ...
    IF ( ALL(soluteSite%q == 0._dp) .OR. ALL(solventSite%q == 0._dp) ) THEN
        IF (verbose) PRINT*,"The electrostatic potential energy is zero."
        RETURN
    END IF
    
    ! precompute Rot_ij(omega,psi)*k_solv(a) for speeding up
    DO CONCURRENT (m=1:SIZE(solventSite), p=1:molRotGrid%n_angles, o=1:angGrid%n_angles)
    ...
    DO CONCURRENT (s=1:nb_species, i=1:spaceGrid%n_nodes(1), j=1:spaceGrid%n_nodes(2), k=1:spaceGrid%n_nodes(3), &
        ...
        DO CONCURRENT (m=1:SIZE(solventSite), n=1:SIZE(soluteSite), (solventSite(m)%q/=0._dp .AND. soluteSite(n)%q/=0._dp))
            ...
            IF (xuv2 < hardShellRadiusSQ) THEN
                ...
                CYCLE
            ELSE
                V_psi = V_psi + qfact*soluteSite(n)%q*solventSite(m)%q/SQRT(xuv2)
            END IF
        END DO
