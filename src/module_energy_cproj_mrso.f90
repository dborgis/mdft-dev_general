module module_energy_cproj_mrso
  ...
  type :: p3_type
    ...
    real(dp), allocatable :: harm_sph_huge(:,:,:,:) ! itheta,m,mup,mu
...
contains
  ...
  subroutine energy_cproj_mrso (ff,df)
    ...
    if (.not. allocated(p3%harm_sph)) then
      ...
      ! allocate ( p3%harm_sph_huge(ntheta,0:mmax,-mmax:mmax,-mmax:mmax), source=0._dp)
      do p=1,np
        m = p3%m(p)
        mup = p3%mup(p)
        mu = p3%mu(p)
      ...
      !     do mu=-m,m
      !       do i=1,ntheta
      !         p3%harm_sph_huge(i,m,mup,mu) = harm_sph
    end if
    ...
    do ix_q=1,nx
      do iy_q=1,ny
        do iz_q=1,nz
          ...
          ! if(q_eq_mq) then
          !   print*, "BEGGINING OF OZ, deltarho_p_q and deltarho_p_mq="
          !   print*, deltarho_p_q-deltarho_p_mq
          !   print*,
          ! end if
          ...
          ! if(q_eq_mq) then
          !   print*, "in mol frame, deltarho_p_q and deltarho_p_mq="
          !   print*, deltarho_p_q- deltarho_p_mq
          !   print*,
          ! end if
...
! end if
          ...
          ! if (q_eq_mq) then
          !   print*, "q=-q gamma(q) and gamma(mq)="
          !   print*, gamma_p_q
          !   print*, gamma_p_mq
          !   print*,
          ! end if
...
stop "megaman"
    ...
    function angl2proj (foo_o) result (foo_p)
      ...
      ! block
      ! !   complex(dp) :: foo_p2
      !   complex(dp) :: foo_p_slow, sum_of_diff
      !   sum_of_diff = zeroc
      !   do m=0,mmax
      !     do mup=-m,m
      !       do mu=0,m,mrso
      ...
      !           o = grid%indo(itheta,iphi,ipsi)
      !
      !           foo_p_slow = foo_p_slow&
      !           + fm(m)*p3%harm_sph(itheta,p)*grid%w(o)/sum(grid%w)*foo_o(o)*exp(ii*mup*grid%phi(o) +ii*mu*grid%psi(o))
      !
      !           ! ! pour tester je calcule ici deltarho_p(m,-mup,-mu) qui doit être = (-1)**(mup+mu)*conjg(deltarho_p(m,mup,mu))
      !           ! foo_p2 = foo_p2&
      !           ! + fm(m)*harm_sph(m,-mup,-mu,theta(itheta))*grid%w(o)/sum(grid%w)*foo_o(o)&
      !           !   *exp(-ii*mup*grid%phi(o) -ii*mu*grid%psi(o))
      !         end do
      !         ! print*, foo_p2 - (-1)**(mup+mu)*conjg(foo_p(p))
      !         sum_of_diff = sum_of_diff + foo_p_slow - foo_p(p)
      !       end do
      !     end do
      !   end do
      !   print*, "verifying angl2proj: sum_of_diff=",sum_of_diff
