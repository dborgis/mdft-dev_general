...
implicit none
real(kind=dp) :: mean_density !>@var mean density in plan xy for each z
real(kind=dp),dimension(nfft1,nfft2,nfft3), intent(in) :: array
integer(kind=i2b) :: k !>@var dummy
real(kind=dp) :: z
...
mean_density=0.0_dp
do k=1,nfft3
  z = real(k-1,kind=dp) * deltaz
  mean_density = sum(array(:,:,k)) / real(nfft1*nfft2,kind=dp)
