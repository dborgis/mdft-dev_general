...
!===================================================================================================================================
  SUBROUTINE read_hard_sphere_radius_and_allocate_if_necessary
    ...
    USE system           ,ONLY: n_0_multispec, temperature, nb_species, spaceGrid, solventSite
        ...
        IF ( input_line (i) (1:j) == 'hard_sphere_radius' ) THEN
            DO s = 1 , nb_species
                ...
                IF ( hs(s)%R <= 0.0_dp ) THEN ! check if one radius is negative, ie if one has to compute wca diameter
                    CALL compute_wca_diameter ( n_0_multispec(s) , temperature, solventSite(s)%sig , solventSite(s)%eps , d_wca )
                    hs(s)%R = d_wca / 2.0_dp
                END IF
            END DO
            EXIT
