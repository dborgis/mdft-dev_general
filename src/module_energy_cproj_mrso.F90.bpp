...
!$SH for mmax in $(seq 1 5); do
  use module_rotation, only: rotation_matrix_between_complex_spherical_harmonics_lu_${mmax}
!$SH done
...
!$SH      for mrso in 1 2; do
      case(${mrso})
        if(present(print_timers)) then
          call energy_cproj_mrso_${mmax}_${mrso}( ff, df, print_timers)
        else
...
!$SH for mmax in $(seq 1 5); do
!$SH for mrso in 1 2; do
  subroutine energy_cproj_mrso_${mmax}_${mrso} ( ff, df, print_timers)
    ...
    real(dp) :: vexc(no), rho, xi, q(3), lx, ly, lz, rho0, theta(ntheta), wtheta(ntheta), dv, kT
    real :: total_time_in_subroutine
    complex(dp) :: R_loc(-mmax:mmax), deltarho_p_q_loc, deltarho_p_mq_loc
    integer :: ip2_loc(-mmax:mmax)
    complex(dp) :: R(0:mmax,-mmax:mmax,-mmax:mmax) ! Table of generalized spherical harmonics of m, mup, mu
    complex(dp) :: gamma_p_q(np), gamma_p_mq(np), deltarho_p_q(np), deltarho_p_mq(np), gamma_m_khi_mu_q, gamma_m_khi_mu_mq
    ...
    !$OMP PARALLEL DO DEFAULT(FIRSTPRIVATE) SHARED(gamma_p_isok,cq,deltarho_p,grid,ck,p3)
    do iz_q=1,nz/2+1
      ...
      do iy_q=1,ny
        ...
        do ix_q=1,nx
          ...
          !
          R = rotation_matrix_between_complex_spherical_harmonics_lu_${mmax} (q)
...
function angl2proj_${mmax}_${mrso} (foo_o) result (foo_p)
  ...
  complex(dp) :: foo_theta_mu_mup(1:ntheta,0:mmax/mrso,-mmax:mmax)
