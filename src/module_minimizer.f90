...
contains
    ...
    subroutine init_lbfgsb
        ...
        if (.not. allocated(solvent(1)%rho) ) then
            print*, "probleme dans module_minimizer > init_lbfgsb"
            print*, "solvent is allocated but not solvent%rho"
            error stop
        end if
                    ...
                    do ix=1,grid%nx
                        do io=1,grid%no
                            ...
                            if (solvent(is)%vext(io,ix,iy,iz) < solvent(is)%vext_threeshold) then
                                ...
                                lbfgsb%l(icg) = 0._dp!solvent(is)%rho(ix,iy,iz,io)
                                ! lbfgsb%u(icg) = 0._dp!solvent(is)%rho(ix,iy,iz,io) ! lower and upper bounds the same, I hope lbfgsb understands this means don't touch to this?
                            end if
                        end do
                    end do
                end do
