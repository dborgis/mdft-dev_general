    ...
    do k=1, nfft3
      do o=1,nb_omega
        do p=1,nb_psi
          ...
          dF(icg)=dF(icg)+kBT*psi*deltaV*weight(o)*weight_psi(p)*rho_0*(&
                  ...
                  kBT*psi*weight(o)*weight_psi(p)*rho_0*deltaV*(FGxx(i,j,k)+FGyy(i,j,k)+FGzz(i,j,k)+2.0_dp*FGxy(i,j,k)&
                 +2.0_dp*FGxz(i,j,k)+2.0_dp*FGyz(i,j,k)+2.0_dp*costheta0*(FGx(i,j,k)+FGy(i,j,k)+FGz(i,j,k))+costheta0**2*FG0(i,j,k))



!          dF(icg)=dF(icg)+kbT*deltaV*weight(o)*weight_psi(p)*rho_0*lambda_w*psi*((Fxx(i,j,k)**2+Fyy(i,j,k)**2+Fzz(i,j,k)**2+&
...
!          dF(icg)=dF(icg)+kbT*deltaV*weight(o)*weight_psi(p)*rho_0*lambda_w*psi*((FAxx(i,j,k)**2+FAyy(i,j,k)**2+FAzz(i,j,k)**2+&
!                 2.0_dp*(FAxy(i,j,k)**2+FAxz(i,j,k)**2+FAyz(i,j,k)**2)-2.0_dp*costheta0*(FAx(i,j,k)**2+FAy(i,j,k)**2+FAz(i,j,k)**2)&
!                  +costheta0**2*FA0(i,j,k)**2)&
...
!              -2.0_dp*costheta0*(FAx(i,j,k)*Ax(i,j,k)+FAy(i,j,k)*Ay(i,j,k)+FAz(i,j,k)*Az(i,j,k))+costheta0**2*FA0(i,j,k)*A0(i,j,k)))
          ...
          do n=1,nb_solute_sites
            dF(icg)=dF(icg)+lambda1_mol(n)*kBT*psi*weight(o)*weight_psi(p)*((Hxx(n)*DHxx(i,j,k,n)+Hyy(n)*DHyy(i,j,k,n)+&
Hzz(n)*DHzz(i,j,k,n)&
...
real(kind = dp) :: f_ww, r, rmin, rsw, rmax

real(kind = dp) , parameter :: gam = 2.0_dp/3.0_dp

real(kind = dp) :: deltar, exp_term, Switch
