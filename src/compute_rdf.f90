...
delta_r = RdfMaxRange / real ( nbins , kind = dp ) ! Width of each bin of the histogram
...
do species = 1 , nb_species
  ...
  do n = 1 , nb_solute_sites
    ...
    do i = 1 , nfft1
      ...
      xnm2 = ( real ( i - 1 , kind = dp ) * deltax - x_mol ( n ) ) ** 2

      do j = 1 , nfft2

        ynm2 = ( real ( j - 1 , kind = dp ) * deltay - y_mol ( n ) ) ** 2

        do k = 1 , nfft3

          znm2 = ( real ( k - 1 , kind = dp ) * deltaz - z_mol ( n ) ) ** 2
  ...
  where ( recurrence_bin /= 0 )
     ...
     rdf = rdf / real ( recurrence_bin , kind = dp )
  ...
  do n = 1 , nb_solute_sites
     ...
     do bin = 0 , nbins
           ...
           write ( 10 , 100 ) (real ( bin , kind = dp )) * delta_r , rdf ( n , bin ) 
     end do
