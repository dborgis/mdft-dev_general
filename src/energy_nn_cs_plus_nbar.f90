...
!   real(dp) :: k2_loc, c_s_loc ! dummy local k2
...
!   real(dp) :: lognbar ! dummy for log(nbar)
  real(dp), intent(out) :: Fint ! excess energy one wants to compute here
...
!         V_nk(l,m,p) = c_s_loc * ( delta_nk(l,m,p) - delta_nbark(l,m,p) )
!
!       END DO
!     END DO
!   END DO
...
!
!     call cpu_time ( time1 )
stop "energy_nn_cs_pus_nbar desactivated since o,p=>io"
