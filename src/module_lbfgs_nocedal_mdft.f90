module module_lbfgs_nocedal_mdft
  ...
  use precision_kinds, only: mydp=>dp ! The usual varname "dp" in MDFT conflicts with one that is used in the core L-BFGS-B routine by Nocedal.
  use module_blas_etc
  private
  ...
  type lbfgsb_type
      ...
      real(mydp)               :: factr  = 0.001_mydp/epsilon(1._mydp) ! The iteration will stop when
      ...
      real(mydp)               :: pgtol  = 0.001_mydp ! The iteration will stop when max{|proj g_i | i = 1, ..., n} <= pgtol
  ...
  subroutine init_lbfgsb
    ...
    ! subroutine setulb(n, m, x, l, u, nbd, f, g, factr, pgtol, wa, iwa, task, iprint, csave, lsave, isave, dsave) MAX 9 MAI 2016 SINCE WE ALWAYS DO UNCONSTRAINED MINIMIZATION
      subroutine setulb(n, m, x,            f, g, factr, pgtol, wa, iwa, task, iprint, csave, lsave, isave, dsave)
        use precision_kinds, only: myprecision=>dp
        implicit none
...
subroutine cmprlb(n, m, x, g, ws, wy, sy, wt, z, r, wa, index,  theta, col, head, nfree, cnstnd, info)
  ...
  70  continue
      ...
      real(myprecision) , parameter :: zero=0.
  ...
  24  continue
   ...
   30    continue
      ...
      real(myprecision), parameter :: one=1.,zero=0.,big=1._myprecision+10
      real(myprecision), parameter :: ftol=0.001,gtol=0.9,xtol=0.1
 ...
 556  continue
  ...
  41        continue
      ...
      real(myprecision) , parameter :: one=1.
...
subroutine subsm ( n, m, nsub, ind, x, d, xp, ws, wy, theta, xx, gg, col, head, iword, wv, wn, iprint, info )
  ...
  1004 format (/,'----------------exit SUBSM --------------------',/)
      ...
      real(myprecision), parameter ::  zero=0.,p5=0.5,p66=0.66
      real(myprecision), parameter ::  xtrapl=1.1,xtrapu=4.0
 ...
 1000 continue
      ...
      real(myprecision) , parameter :: zero=0.,p66=0.66,two=2.,three=3.
