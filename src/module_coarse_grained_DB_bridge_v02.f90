...
contains
    ...
    subroutine init
        ...
        real(dp) :: c00000, kT, n0, cgb_B6, cgb_sigma
        integer :: is, ns
        integer :: nx, ny, nz
        ...
        B6 = getinput%dp('cgb_B6', defaultvalue=0._dp)
        sigma = getinput%dp('cgb_sigma', defaultvalue=1._dp)
        if(new_version) then
            write(*,*) 'cgb: new WDA version'
        else
            write(*,*) 'cgb: gageat version'
        endif
        ...
        !! TODO: l'extraire du fichier c directement mais cela implique de trop grosses modifs pour les permiers tests :)
          ...
          if( solvent(1)%name == "spce" )  c00000 = -14.64872
          if( solvent(1)%name == "tip3p" ) c00000 = -11.9078
          A3 =  1.0_dp - c00000/2.0_dp
          write(*,*) "A3 = ",A3, ' B6 =', B6, ' sigma =', sigma
    ...
    subroutine fill_kernel
        ...
        !!!!!                                                          !!!!!
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        real(dp) :: pi, gaussianPrefactor, oneOverTwoSigmaSquare, dSquareCutoff
        ...
        do iz = 1, nz
            write(1001,*) grid%dz * iz, kernel(1,1,iz)
        end do
      ...
      do iy=1,ny
        do ix=1,nx
         ...
         if( n_cg < one )then
              ...
              dfb(ix,iy,iz) =  kT * ( deltaN * dfb(ix,iy,iz) + Phib )
