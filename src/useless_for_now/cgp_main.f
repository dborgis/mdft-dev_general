...
c       --------------------------------------------------------------------
c 
	subroutine          driver(ndim,x,f,g,eps)
c
...
c
c        EPS= 1.0D-5 declared in module cg == EPSG
...
c Calculate the function and gradient values here
	...
	call interface_compute_energy_and_gradients(n,x,f,g,stopouencore)
        IF(STOPOUENCORE(1:4)=='STOP') THEN
           IFLAG=-1
           WRITE (*,910)
           GO TO 50
        ENDIF
...
c
 ...
 910    format (/,' convergence failed in less than itermax iterations')
	end
