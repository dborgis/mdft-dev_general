...
subroutine energy_hydro_chandler_HSB

  use precision_kinds , only : dp , i2b
  use system , only : nfft1 , nfft2 , nfft3 , deltaV, rho_0 , nb_k , c_s , kBT , delta_k , nb_omega , nb_species,n_0,nb_psi,&
  Lx,Ly,Lz,radius, c_s_hs, muexc_0_multispec
  use constants , only : fourpi , i_complex,twopi, pi
  ...
  real(kind=dp), allocatable, dimension(:,:,:) :: V_nbar,V_lr , V_lrbar! coarse grained V_n
  complex(kind=dp), allocatable, dimension(:,:,:) :: V_nbark , V_lrk, V_lrbark! FFT(V_n)
  ...
  real(kind=dp) :: Fint,Flrint ! excess energy one wants to compute here
  real(kind=dp) :: Vint, Vlrint ! dummy part of Fint
  ...
  complex(kind=dp), allocatable, dimension(:,:,:) :: dF_cgk, dF_CSk, dFb_lrk ! coarse grained part of dF
  real(kind=dp), allocatable, dimension(:,:,:) :: dF_cg , dF_CS! coarse grained part of dF
  real(kind=dp) :: time0 , time1 ! timesteps
  real(kind=dp) :: R_cg ! radius of the coarse graining gaussian function  
  real(kind=dp) :: dF_cg_ijk, dFB_lr ! dummy for local dF_cg
  real(kind=dp) :: delta_n_ijk, Fb_lr ! dummy for local delta_n 
  real ( kind = dp ) :: psi ! local cg_vect
  real (kind=dp ) :: Vol_HS  , FF_CS_nbar , eta_bar, eta_0
  real (kind=dp ) :: Fexc_CS, dFexc_CS
  ...
  Vol_HS=4.0_dp/3.0_dp*Pi*radius(1)**3  !!! Warning this subroutine works whith 1 solvent specie only for the moment
  eta_0=Vol_HS*n_0

  !TEST ZONE START
  d_0 = d_0
  gamma_0 = gamma_0*1.25_dp!/10_dp  !10. !10  22 is good for R=10A
  R_cg = R_cg!*2.0_dp
  !TEST ZONE STOP

  print*,'size c_s_hs =', size(c_s_hs)
  !stop
  ...
  allocate(V_lrk(nfft1/2+1,nfft2,nfft3))  
  do p = 1 , nfft3
    do m = 1 , nfft2
      do l = 1 , nfft1 / 2 + 1
        ...
        V_lrk ( l , m , p ) = c_s_hs ( k_index ) * (delta_nbark ( l , m , p ) )  
  ...
  allocate( V_lrbark ( nfft1 / 2 + 1 , nfft2 , nfft3 ) )
  V_lrbark = V_lrk * exp(prefactor*k2)
  ...
  deallocate(V_lrk)
  call dfftw_execute(plan_backward)
  allocate(V_lr(nfft1,nfft2,nfft3))
  V_lr = out_backward/Nk
  ...
  in_backward = V_lrbark
  deallocate(V_lrbark)
  call dfftw_execute(plan_backward)
  allocate(V_lrbar(nfft1,nfft2,nfft3))
  V_lrbar = out_backward/Nk
  ...
  allocate ( dF_CS(nfft1, nfft2, nfft3))
  F_cg = 0.0_dp
  dF_CS=0.0_dp 
  do i=1,nfft1
    do j=1,nfft2
      do k=1,nfft3
        ...
        eta_bar = Vol_HS*nbar*n_0
        if (nbar==0.0) stop 'problem in cs_plus_hydro.f90 : nbar should not be =0.0. STOP'
        lognbar = log(nbar)
  
        FF_CS_nbar = FF_CS_nbar +KbT/Vol_HS*(eta_bar*Fexc_CS(eta_bar))*deltaV  !-eta_0*Fexc_CS(eta_0)

        dF_CS(i,j,k) = (eta_bar*dFexc_CS(eta_bar)+Fexc_CS(eta_bar))*kBT*deltaV*n_0!/Vol_HS
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        F_cg = F_cg + DeltaV*(6.0*gamma_0/d_0)*(nbar-1.0_dp)**2*nbar**2 - (mu_0)*n_0*(nbar-1.0_dp)*deltaV+&
        muexc_0_multispec(1)*nbar*deltaV*n_0
  
        ! + ideal part associated to nbar
        F_cg = F_cg - kBT*fact_n*(nbar*lognbar- nbar + 1.0_dp)
  
        dF_cg(i,j,k) = DeltaV*(6.0*gamma_0/d_0)*2.0_dp*(nbar-1.0_dp)*nbar*(2.0_dp*nbar-1.0_dp) -&
                       (mu_0)*n_0*deltaV- kBT*fact_n*lognbar +muexc_0_multispec(1)*n_0*deltaV
...
print*, 'FF_CS_nbar =',FF_CS_nbar 

!large gradient part
  S_cg = 3.0_dp/2.0_dp*d_0*gamma_0*DeltaV *sum( gradx_delta_nbar**2 + grady_delta_nbar**2&
                  + gradz_delta_nbar**2 )

  print*, 'prefacteur grad',3.0_dp/2.0_dp*d_0*gamma_0*DeltaV 
  ...
  in_forward=dF_CS
  call dfftw_execute (plan_forward)
  allocate(dF_CSk(nfft1/2+1, nfft2, nfft3))
  dF_CSk=out_forward
  
  ! define coarse grained gradients in k space
  dF_cgk = ( dF_cgk + dS_cgk - dF_CSk ) * exp ( prefactor * k2 )
  deallocate(dS_cgk)
  deallocate(dF_CSk)
  ...
  ! following count does not take into account all cases where nb_omega /=1
  if ( nb_omega /= 1 ) then
    print *, 'nb_omega /= 1 and hydrophobic part is calculated while not still implemented'
  ...
  do i = 1, nfft1
    do j = 1, nfft2
      do k = 1, nfft3
        ...
        Vlrint= kBT * n_0 * fact_n * ( V_lr(i,j,k) - V_lrbar(i,j,k) ) 
        Fint = Fint + 0.5_dp* ( Delta_n (i,j,k) - Delta_nbar (i,j,k) ) * Vint ! TODO doesnt work in case of nb_omega /= 1)
        Flrint=Flrint + 0.5_dp * delta_nbar(i,j,k)*Vlrint
        dF_cg_ijk = dF_cg(i,j,k)
        do o = 1, nb_omega
          do p=1,nb_psi
            icg = icg + 1
            psi = cg_vect ( icg )
            dF (icg) = dF ( icg )+2.0_dp*psi*weight_psi(p)*weight(o)/(fourpi*twopi/sym_order)*( Vint + dF_cg_ijk +Vlrint)
          end do
        end do
      end do
    end do
  ...
  ! conclude
  FF = FF + Fint + F_cg + S_cg - FF_CS_nbar+Flrint
...
function Fexc_CS(eta)
implicit none
double precision :: eta, Fexc_CS

Fexc_CS = eta*(4.0d0 - 3.0d0*eta)/(1.0d0 - eta)**2

end function Fexc_CS

function dFexc_CS(eta)
implicit none
double precision :: eta, dFexc_CS

dFexc_CS = (4.0d0 - 6.0d0*eta)/(1.0d0 - eta)**2 + 2.0d0*eta*(4.0 - 3.0*eta)/(1.0d0-eta)**3

end function dFexc_CS
