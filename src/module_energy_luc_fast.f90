module tableaux_dft_3d_fast
  ...
  COMPLEX(dp),DIMENSION(:,:,:),ALLOCATABLE:: delta_rho,delta_rho_proj1,auxi_1,auxi_2
  COMPLEX(dp),DIMENSION(:,:,:),ALLOCATABLE:: gamma4_proj1
...
deallocate(delta_rho_k_angle)
  ...
  subroutine luc_oz (qvec, delta_rho_proj, gamma4_proj)
    use precision_kinds, only: dp
    USE lecture                                    ! pour lire des valeurs a la Luc
    USE tableaux_dft_3d_fast                            ! definit le type des tableaux de valeurs a partager
    ...
    complex(dp), intent(in) :: delta_rho_proj(0:grid%mmax,-grid%mmax:grid%mmax,-grid%mmax/2:grid%mmax/2)
    complex(dp), intent(out) :: gamma4_proj(0:grid%mmax,-grid%mmax:grid%mmax,-grid%mmax/2:grid%mmax/2)
    CHARACTER(50)::nomfic,texte
    CHARACTER(10) texte1
    COMPLEX(dp)::xi_cmplx,auxi_cmplx,ck_cmplx,coeff_cmplx,coeff1_cmplx,coeff2_cmplx
    integer :: mmax, mmax2, mmax_ck, mmax2_ck, mu, nu, i, ialp, ialpmax, iq, iqmax, k, khi, l, m, n, m2, mu1, mu2, n2, nu2
    fac(0)=1.
    do k=1,50
...
gamma4_proj1(m,khi,mu2)=gamma4_proj1(m,khi,mu2)+(-1)**khi *tab5(m,n,khi,mu2,nu2,iq) *delta_rho_proj1(n,khi,-nu2)
  ...
  function symbol_3j(m,n,l,mu,nu,lu)
    ...
    !
    IF(itriangle(m,n,l).eq.0.or.mu+nu+lu.NE.0.or. ABS(mu).gt.m.or.abs(nu).gt.n.or.abs(lu).gt.l) then
      symbol_3j=0.
    else
      som=0.
  ...
  function harm_sph(m,mu,mup,beta)
    ...
    implicit real(dp) (a-h,o-z)
    integer :: it, l, mu0, i
    integer, intent(in) :: m, mu, mup, beta
    !
  ...
  subroutine gauleg(x,w,n)
    ...
    do i=1,m                        ! on s'interesse au i�me zero du polynome Pn(x) de Legendre
      ...
      do j=1,n
        p3=p2
        p2=p1
        p1=((2*j-1._dp)*xi*p2-(j-1._dp)*p3)/j    ! relation de r�currence entre les Pj
      end do
...
end module module_energy_luc_fast
